    ...
    def planning(self):
        for i in range(self.iter_max):
            ...
            if node_new and not self.utils.is_collision(node_near[0], node_new[0])and not self.utils.is_collision(node_near[1], node_new[1])and not self.utils.is_collision(node_near[2], node_new[2])and not self.utils.is_collision(node_near[3], node_new[3]):
                # if 1 < np.linalg.norm(node_new[0],node_new[1]) < 8 and 1 < np.linalg.norm(node_new[0],node_new[2]) < 8 and 1 < np.linalg.norm(node_new[0],node_new[3]) < 8 and 1 < np.linalg.norm(node_new[1],node_new[2]) < 8 and 1 < np.linalg.norm(node_new[1],node_new[3]) < 8 and 1 < np.linalg.norm(node_new[2],node_new[3]) < 8:
                # print(node_new[0].x,node_new[0].y)
                # bet_dist1 = np.linalg.norm(np.array([node_new[0].x,node_new[0].y]),np.array([node_new[1].x,node_new[1].y]))
                # bet_dist2 = np.linalg.norm(np.array([node_new[0].x,node_new[0].y]),np.array([node_new[2].x,node_new[2].y]))
                # bet_dist3 = np.linalg.norm(np.array([node_new[0].x,node_new[0].y]),np.array([node_new[3].x,node_new[3].y]))
                # bet_dist4 = np.linalg.norm(np.array([node_new[1].x,node_new[1].y]),np.array([node_new[2].x,node_new[2].y]))
                # bet_dist5 = np.linalg.norm(np.array([node_new[1].x,node_new[1].y]),np.array([node_new[3].x,node_new[3].y]))
                # bet_dist6 = np.linalg.norm(np.array([node_new[2].x,node_new[2].y]),np.array([node_new[3].x,node_new[3].y]))
                bet_dist1 = math.hypot(node_new[0].x - node_new[1].x, node_new[0].y - node_new[1].y)
                bet_dist2 = math.hypot(node_new[0].x - node_new[2].x, node_new[0].y - node_new[2].y)
                bet_dist3 = math.hypot(node_new[0].x - node_new[3].x, node_new[0].y - node_new[3].y)
                bet_dist4 = math.hypot(node_new[1].x - node_new[2].x, node_new[1].y - node_new[2].y)
                bet_dist5 = math.hypot(node_new[1].x - node_new[3].x, node_new[1].y - node_new[3].y)
                bet_dist6 = math.hypot(node_new[2].x - node_new[3].x, node_new[2].y - node_new[3].y)
                dist_low = 1
                dist_high = 6
                if dist_low < bet_dist1 < dist_high and dist_low<bet_dist2 < dist_high and dist_low<bet_dist3<dist_high and dist_low<bet_dist4<dist_high and dist_low<bet_dist5<dist_high and dist_low<bet_dist6<dist_high:
...
def main():
    ...
    x1_start = (x_s, y_s)  # Starting node
    x1_goal = (x_g, y_g)  # Goal node
    
    x2_start = (x_s+2, y_s)  # Starting node
    x2_goal = (x_g+2, y_g)  # Goal node
    
    x3_start = (x_s+2, y_s+2)  # Starting node
    x3_goal = (x_g+2, y_g+2)  # Goal node
    
    x4_start = (x_s, y_s+2)  # Starting node
    x4_goal = (x_g, y_g+2)  # Goal node
    M_start = [x1_start,x2_start,x3_start,x4_start]
    M_goal = [x1_goal,x2_goal,x3_goal,x4_goal]
    
    marrt = MARRT(M_start, M_goal, 0.5, 0.05, 200000)
    path = marrt.planning()
